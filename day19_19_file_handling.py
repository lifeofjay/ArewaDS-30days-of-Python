...
def count_lines_and_words(text):
    ...
    return line_count, word_count
merlina_trump = """Thank you very much. Thank you. You have all been very kind to Donald and me, to our young son Barron, and to our whole family. It's a very nice welcome and we're excited to be with you at this historic convention. I am so proud of your choice for president of the United States, my husband, Donald J. Trump.
...
def count_language_occurrences(data):
    language_counts = {}
    for country in data:
...
language_counts = count_language_occurrences(data)
...
with open("email_exchange_big.txt", 'r') as file:
    ...
    incoming_emails = re.findall(email_pattern, email_text)
...
def find_most_common_words(data, num_words):
    try:
        if isinstance(data, str) and data.endswith('.txt'):
            ...
            raise ValueError("Invalid input. Please provide a string or a file path.")
...
result = find_most_common_words(text, 5)
print(result)
...
result_obama_speech = find_most_frequent_words(obama_speech, 10)

print("The ten most frequent words in the Obama's speech:")
for i, (word, frequency) in enumerate(result_obama_speech, 1):
    print(f"{i}. {word}: {frequency} times")
    
#b) The ten most frequent words used in Michelle's speech 
michelle_obama_speech = """ As you might imagine, for Barack, running for president is nothing compared to that first game of basketball with my brother, Craig.
...
result_michelle_speech = find_most_frequent_words(michelle_obama_speech, 10)

print("The ten most frequent words in the Michelle's speech:")
for i, (word, frequency) in enumerate(result_michelle_speech, 1):
    print(f"{i}. {word}: {frequency} times")
# c) The ten most frequent words used in Trump's speech 
trump_speech = """ Chief Justice Roberts, President Carter, President Clinton,President Bush, fellow Americans and people of the world – thank you.
...
result_trump_speech = find_most_frequent_words(trump_speech, 10)

print("The ten most frequent words in the Trump's speech:")
for i, (word, frequency) in enumerate(result_trump_speech, 1):
    print(f"{i}. {word}: {frequency} times")
...
result_melina_speech = find_most_frequent_words(melina_speech, 10)

print("The ten most frequent words in the Melina's speech:")
for i, (word, frequency) in enumerate(result_melina_speech, 1):
    print(f"{i}. {word}: {frequency} times")
import re
from collections import Counter
...
def remove_support_words(text, support_words):
    ...
    return " ".join([word for word in words if word not in support_words])

def check_text_similarity(michelle_obama_speech, melina_speech):

    support_words = ["the", "a", "an", "in", "on", "of", "to", "and", "is", "are", ...] 

    def get_word_counts(text):
        words = remove_support_words(text, support_words).split()
        return Counter(words)

    words1 = get_word_counts(michelle_obama_speech)
    words2 = get_word_counts(melina_speech)

    all_words = set(words1.keys()) | set(words2.keys())
    dot_product = sum(words1.get(word, 0) * words2.get(word, 0) for word in all_words)
    magnitude1 = sum(count * count for count in words1.values()) ** 0.5
    magnitude2 = sum(count * count for count in words2.values()) ** 0.5
    similarity = dot_product / (magnitude1 * magnitude2)

    return similarity

similarity = check_text_similarity(michelle_obama_speech, melina_speech)
print("Similarity:", similarity)

#Find the 10 most repeated words in the romeo_and_juliet.txt
romeo_and_juliet = """ The Project Gutenberg EBook of Romeo and Juliet, by William Shakespeare
...
python_lines = sum(1 for line in text.splitlines() if "python" in line.lower())

print(f"Number of lines containing 'python' or 'Python': {python_lines}")
#Count the number lines containing JavaScript, javascript or Javascript
javascript_lines = sum(1 for line in text.splitlines() if "javascript" in line.lower())

print(f"Number of lines containing 'JavaScript': {javascript_lines}")
#Count the number lines containing Java and not JavaScript
java_lines = sum(1 for line in text.splitlines() if "java" in line.lower())

javascript_lines = sum(1 for line in text.splitlines() if "javascript" in line.lower())

java_not_javascript_lines = java_lines - javascript_lines

print(f"Number of lines containing 'Java' and not 'JavaScript': {java_not_javascript_lines}")
